...
class Common_model extends CI_Model {
	...
	//================for checkbox=====================//
    function leftpaneldata(){
		$this->db->where('status','1');
		$qry=$this->db->get('sidepanel');
		$val=$qry->result();
		$val['num_rows']=$qry->num_rows();
	...
	function activeAdmin($table,$activeadmin){
		//echo "<pre>";print_r($activeadmin);die;
		$this->db->where_in('id',$activeadmin);
		$query = $this->db->get($table);
		$result['num_row'] = $query->num_rows();
        $result['result'] = $query->result();
	...
	function inactiveAdmin($table,$whereArray,$inactiveadmin){
		//echo "<pre>";print_r($inactiveadmin);die;
	
		 if(!empty($inactiveadmin)){
		   $this->db->where_not_in('id',$inactiveadmin);
		 }
		$this->db->where($whereArray);
		$query = $this->db->get($table);
		$result['num_row'] = $query->num_rows();
	...
	function getEmailTemplate($emailId, $replace_placeholderArr)
	{
		$finalarray = array();
        //echo $emailId;
		//echo "<pre>";print_r($replace_placeholderArr);die;
        //======== get template deatils ========//
		$this->db->where('id',$emailId);
		$query = $this->db->get('email_template');
        $emailTemplateArr = $query->result();
		//echo "<pre>";print_r($emailTemplateArr);die;
		$emailSubject = $emailTemplateArr[0]->subject;
		$emailBody = $emailTemplateArr[0]->body;
		//echo $emailBody;die;
		//======== get template deatils ========//
        ...
        $site_details = $this->Common_model->getAllDataWithMultipleWhere('site_settings',$whereSitesettingsArray,'id');
		//echo "<pre>";print_r($site_details);die;
		$sitename = $site_details['result'][0]->site_name;
		$email = $site_details['result'][0]->email;
		...
		//$generalArr = array($contactEmail,$siteurl,$sitename,$logo,$copyr8);
		$generalArr = array('[Email]'=>$email,
							'[SiteName]'=>$sitename);
		//echo "<pre>";print_r($generalArr);die;
		...
		$placeholderArr = array('[USER]','[LINK]','[Email]','[SiteName]',);
		$emailSubjectRplc = str_replace($placeholderArr,$finalArray,$emailSubject);
		$emailBodyRplc = str_replace($placeholderArr,$finalArray,$emailBody);
		//echo $emailSubjectRplc;
		//echo $emailBodyRplc;die;
        return $emailSubjectRplc.'[ESOLZ]'.$emailBodyRplc.'[ESOLZ]'.$sitename;
    }
	
	function getEmailTemplateforCupon($emailId, $replace_placeholderArr)
	{
		$finalarray = array();
        //echo $emailId;
		//echo "<pre>";print_r($replace_placeholderArr);die;
        //======== get template deatils ========//
		$this->db->where('id',$emailId);
		$query = $this->db->get('email_cupon_template');
        $emailTemplateArr = $query->result();
		//echo "<pre>";print_r($emailTemplateArr);die;
		$emailSubject = $emailTemplateArr[0]->subject;
		$emailBody = $emailTemplateArr[0]->body;
		...
		$placeholderArr = array('[USER]','[LINK]','[CUPON]','[Email]','[SiteName]',);
		$emailSubjectRplc = str_replace($placeholderArr,$finalArray,$emailSubject);
		$emailBodyRplc = str_replace($placeholderArr,$finalArray,$emailBody);
		//echo $emailSubjectRplc;
		//echo $emailBodyRplc;die;
...
/************************* Thumbnail Function - Ends *************************/
 ...
 public function find_in_set($table,$searchterm,$whereArr=array()){
        $where = "FIND_IN_SET('".$searchterm."', vendor_id)"; 
        $this->db->where($where); 
...
public function fetch_all_data($table,$condition=array()){
        if(!empty($condition))
        {
            $this->db->where($condition);
        }
...
public function login_user($email,$password,$table){
