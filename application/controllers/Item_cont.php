...
//            $update_data = $this->Common_model->updateDetailsWithMultipleWhere('Item',$whereArray,$data_to_update_price);   
   ...
   // }
    ...
    function calculate(){
        ...
        $UserUnit_id=$this->input->post('unit_id');
...
//         echo $quantity_ajax;
        ...
        if($material_id!="" && ($UserUnit_id!="" && $quantity_ajax!="") )
        {
                ...
                if($unit_details['result'][0]->unit_name =="kg"||$unit_details['result'][0]->unit_name =="gm"||$unit_details['result'][0]->unit_name =="mg")
                {
                    if($unit_details['result'][0]->unit_id==$UserUnit_id)
                    {
                        $price=$material_details['result'][0]->avg_price;
                    }
                    else
                    {
                        $avgPrice=$material_details['result'][0]->avg_price;
                        $toDivide=$unit_details['result'][0]->value;
                        $quantity=1/$toDivide;
                       

                         $whereArray = array('unit_id' =>$UserUnit_id);		
                         $userUnit_details=$this->Common_model->getAllDataWithMultipleWhere('UnitType',$whereArray,'unit_id');
                         $userTodivide=$userUnit_details['result'][0]->value;
                         $UserQuantity=1/$userTodivide;
                         
                      

                         $price=($avgPrice/$quantity)*$UserQuantity;
                ...
                else  if($unit_details['result'][0]->unit_name =="lit"||$unit_details['result'][0]->unit_name =="ml")
                {
                    if($unit_details['result'][0]->unit_name==$UserUnit_id)
                    {
                        $price=$material_details['result'][0]->avg_price;
                    }
                    else
                ...
                else  if($unit_details['result'][0]->unit_name =="pic")
                {
        ...
        $totalPrice=$price*$quantity_ajax;
       ...
       // $material=$this->Common_model->fetchAllData('RawMaterial','material_id','DESC');
		...
		?>
         ...
         <?php  }     
			...
			<div class="col-lg-2" style="margin-top: 1%;" id="actionBtn_<?php echo $nxtCount;?>">
				<input type="button" class="btn btn-success" value="+" onclick="addMore('add', '<?php echo $nxtCount;?>');">
				<?php
				if($nxtCount > 1)
				{
